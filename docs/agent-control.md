...
| `relaunch` | Replace the running agent with a new one in the same endpoint and worktree, on the exact recorded adapter or an explicitly chosen harness, model, effort, and Claude account. | The new agent is alive on the recorded endpoint, and the durable record names the harness that is actually running. |
...
1. **Resolve the profile.**
   ...
   A claude replacement keeps the Claude account its task record names, whatever the relaunching process's environment or the home's current setting, unless `--claude-config-dir` moves it; that override is checked before anything stops ([`docs/configuration.md`](configuration.md) "Claude account").
2. **Safe checkpoint.**
...
- `tests/fm-control-relaunch.test.sh` - the relaunch transaction: identity preservation, harness switching, Claude account preservation and override, the progress note, checkpoint refusals, armed merge-poll preservation, invalid metadata refusal, and rollback after a failed launch.
