...
test_no_profile_keeps_claude_profile_defaults() {
  ...
  expected="env -u CURSOR_AGENT -u CURSOR_INVOKED_AS ENABLE_CLAUDEAI_MCP_SERVERS=false CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false claude --dangerously-skip-permissions \"\$('${ROOT}/bin/fm-operational-input.sh' encode launch-brief < '$HOME_DIR/data/$id/brief.md')\""
...
test_claude_forwards_firstmate_config_dir_when_set() {
  ...
  read_case_record "$rec"
  # The forwarded login folder must exist: fm-spawn refuses a missing one
  # rather than launching Claude on an empty store.
  store="$CASE_DIR/claude-alt"
  mkdir -p "$store"

  out=$(FM_TEST_CLAUDE_CONFIG_DIR="$store" \
    run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR")
  status=$?
  expect_code 0 "$status" "claude spawn with CLAUDE_CONFIG_DIR set should succeed"
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "CLAUDE_CONFIG_DIR='$store' env -u CURSOR_AGENT -u CURSOR_INVOKED_AS ENABLE_CLAUDEAI_MCP_SERVERS=false CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false claude" \
